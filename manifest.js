...
// #2328 (學習路徑警告視窗) start
var moj_time_obj;
var moj_confirm_dialog;

/*Custom 2017-11-29 *049642 (B)*/
var seconds    = parseFloat(moj_auto_logout_time) * 60;
var t1, t2, timerSetReading;
var co_IE8 = (navigator.userAgent.toLowerCase().match(/msie ([\d]+)/));

function countdown()
{
    if(isIE && co_IE8[1] < 10){
        var span_clock = parent.parent.document.getElementById('s_dialog').document.getElementById('span_clock');
    }else{
        var span_clock = parent.parent.s_dialog.document.getElementById('span_clock');
    }

    if (seconds >= 0 && typeof(span_clock) == "object" && span_clock != null)
    {
        span_clock.innerHTML = seconds--;
...
function stop_countdown()
{
    ...
    if(isIE && co_IE8[1] < 10){
        var tmps = parent.parent.document.getElementById('s_dialog').document.body;
    }else{
        var tmps = parent.parent.s_dialog.document.body;
    }
    $(tmps).unblock({});

    parent.parent.document.getElementById('envClassRoom').cols = '266,*';

    moj_warning_start();
    seconds    = parseFloat(moj_auto_logout_time) * 60;
}
...
function logout()
{
    ...
    temp = setTimeout(function () {
        top.document.location.href = "http://"+location.hostname+"/logout.php";
    },1500);

}
function co_go_to_learn(){
    // parent.parent.window.location.href = "/learn/index.php?co=1";

    var label = '';
    // FROM: mooc\public\js\mooc_header.js
    setTimeout(function () {
        $.ajax(
...
function goCourse(csid, env, func) {
    var goSection = func;
    var sysbar = parent.parent.parent.document.getElementById('moocSysbar');

    if ((sysbar !== null) && (typeof sysbar === 'object')) {

        var s_sysbar = parent.parent.document.getElementById('s_sysbar');
        if ((s_sysbar !== null) && (typeof s_sysbar === 'object')) {
            s_sysbar.contentWindow.location.reload(true);
        }
        ...
        moocSysbar.loadSysbar(
            '/learn/goto_course.php', txt,
            function (cid, cname, isTeach, isStudent) {
...
function diolog_alert(){
    ...
    message:'<tr><td><div style="height:220px; background:url(/theme/default/learn/wn_bg.jpg) no-repeat; background-size:cover;"><div class="font01" style="font-family:微軟正黑體;font-size:14px"><p>'+auto_logout+'</p>'+moj_confirm_content+'</div><br><div align="center"><input type="button" value="'+moj_confirm+'" class="cssBtn" onclick="parent.s_catalog.pathtree.stop_countdown();"></div></div><td></tr>',
    css: {
        width: '320px',
        height: '220px'
    }} );
    ...
    t2 = setTimeout(function () {
        // logout();
        co_go_to_learn();
    }, (parseFloat(moj_auto_logout_time) * 60000));
}


function moj_warning_dialog()
{
    if (typeof(moj_confirm_dialog) == 'undefined')
    {
        parent.parent.document.getElementById('envClassRoom').cols = '0,0,*';
        if(isIE){
            if( typeof(parent.parent.document.getElementById('s_dialog').document) == 'undefined'){ //IE
                parent.parent.document.getElementById('s_dialog').src = "/learn/path/co_dialog.html";
            }
        }else{
            if( typeof(parent.parent.s_dialog.document) == 'undefined'){ //IE
                parent.parent.document.getElementById('s_dialog').src = "/learn/path/co_dialog.html";
            }else{
                parent.parent.s_dialog.document.src = "/learn/path/co_dialog.html";
            }
        }

        if (moj_warning_time != 0) {
        /*moj_confirm_dialog = showDialog('moj_confirm_dialog.php', true, window, true, '', '', '400px', '300px');
        moj_warning_start();*/
            setTimeout('diolog_alert()', 1000);
        }
    }
}

function moj_warning_start()
{
    ...
    if (moj_warning_time != 0) {
        moj_warning_stop();
        moj_time_obj = setTimeout('moj_warning_dialog()', moj_warning_time * 60000);
    }
}
function moj_warning_stop()
{
    if (moj_warning_time != 0) {
        clearTimeout(moj_time_obj);
    }
}
// #2328 (學習路徑警告視窗) end/*Custom 2017-11-29 *049642 (E)*/

// 偵測到同一瀏覽器（idx）開啟多個課程-子
function multicourse_alert() {
    ...
    $(tmps).block({
        message:
            '<tr>' +
                '<td>' +
                    '<div style="height:220px; background:url(/theme/default/learn/wn_bg.jpg) no-repeat; background-size:cover;">' +
                        '<br>' +
                        '<div class="font01" style="font-family: 微軟正黑體; font-size: 14px; text-align: left; margin: 2em; line-height: 1.8em;">' +
                            '<p>' + moj_multi_reading + '</p>' +
                        '</div>' +
                        '<br>' +
                        '<div align="center">' +
                            // '<input type="button" value="' + moj_ok + '" class="cssBtn" onclick="window.top.location.reload();">' +
                            // '<input type="button" value="' + moj_ok + '" class="cssBtn" onclick="parent.mooc_sysbar.location.reload();">' +
                            // '<input type="button" value="' + moj_ok + '" class="cssBtn" onclick="parent.s_main.location.replace(\'/learn/path/launch.htm\');">' +
                             '<input type="button" value="' + moj_ok + '" class="cssBtn" onclick="javascript: parent.s_catalog.pathtree.setSessionCid();">' +
                        '</div>' +
                    '</div>' +
                    '<td>' +
            '</tr>',
...
// 偵測到同一瀏覽器（idx）開啟多個課程-母
function moj_warning_multicourse() {
    if (typeof(moj_confirm_dialog) == 'undefined') {
        parent.parent.document.getElementById('envClassRoom').cols = '0,0,*';
        if (isIE) {
            if (typeof(parent.parent.document.getElementById('s_dialog').document) == 'undefined') { //IE
        ...
        if (moj_warning_time != 0) {
            setTimeout('multicourse_alert()', 1000);
        }
    }
}
...
function xmlProcessor()
{
    ...
    if (myPanel.getElementsByTagName('li').length > 0) {
        if (isIE) {
            myPanel.getElementsByTagName('ul')[0].style.margin='0 0 0 -25px';
        } else {
...
*/
    ...
    timerSetReading = window.setInterval("setReading('end', traceReadingIntervalTime);", traceReadingIntervalTime);
    ...
    $('body',parent.catalog).mousemove(function( event ) {
      moj_warning_start();
    });
    $('body',parent.pathtree).mousemove(function( event ) {
        moj_warning_start();
    });
};
    ...
    */
    ...
    for(var i=0; i<nodes.length; i++){
        ...
        if ((ret = generateOrganization(nodes[i])) == '') {
            ...
            if (hasRef && !isDisable)
                htmlCode += '<div class="icon-node"></div><a href="javascript:;" onclick="launchActivity(this,\'' + nodeID + '\',\'' + newTarget + '\');" title="'+htmlTitle+'"><div style="margin:-25px 0 0 58px;">' + nodeTitle + '</div></a>';
            else if (hasRef && isDisable)
                htmlCode += '<a href="javascript:;" onclick="launchActivity(this,\'' + nodeID + '\',\'' + newTarget + '\');" disabled style="cursor:default" title="'+htmlTitle+'"><div style="margin:-25px 0 0 58px;">' + nodeTitle + '</div></a>';
            else
                htmlCode += nodeTitle;
            htmlCode += '</span>' + ret + '</li>';
        }
...
/*
* 加入s_main mousemove 事件
*/
function s_mainMove(){
    setTimeout(function(){
        $('body',parent.parent.s_main.document).mousemove(function( event ) {
            moj_warning_start();
        });
    }, 1000);
}
...
function setReading(type, period) {
    ...
    // if (window.console) {console.log('objForm', objForm);}
    // if (window.console) {console.log('course_id', $(objForm).find("input[name='course_id']").val());}
    // if (window.console) {console.log('username', '');}
    // if (window.console) {console.log('begin_time', $(objForm).find("input[name='begin_time']").val());}
    // if (window.console) {console.log('over_time', '');}
    // if (window.console) {console.log('title', $(objForm).find("input[name='prev_node_title']").val());}
    // if (window.console) {console.log('url', $(objForm).find("input[name='prev_href']").val());}
    // if (window.console) {console.log('activity_id', $(objForm).find("input[name='prev_node_id']").val());}
    ...
    if ( co_switchRPC == undefined || co_switchRPC == '' ) { co_switchRPC = 'N'; }

    // mooc\smarty\templates\learn\download_materials.tpl 也要一起改
    $.ajax({
        url:'/mooc/controllers/course_record.php',
        type:'POST',
        dataType:'json',
        data: {
            action: 'setReading',
            ...
            title: $(objForm).find("input[name='prev_node_title']").val(),
        ...
        success: function(res) {
            ...
            if (res.code <= -90) {
                ...
                moj_warning_multicourse();
                // 停止每分鐘判斷
                clearInterval(timerSetReading);
                // 停止閒置視窗繼續執行
                clearTimeout(moj_time_obj);
            }
        },
...
function setSessionCid() {
    ...
    $.ajax({
        ...
        success: function(res) {
            if (window.console) {console.log('res', res);}
            // parent.s_main.location.replace('/learn/path/launch.htm');

            // 將學習路徑隱藏欄位清除，避免頁面重新整理時，再次更新結束時間
            // var framScatalog = window.frames[2]; // 另外一個FRAME
            var objForm = document.getElementById('fetchResourceForm');
            $(objForm).find("input").val('');
